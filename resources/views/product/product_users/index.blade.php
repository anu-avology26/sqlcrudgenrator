...
<div class="toolbar">
    <a href="{{ route('product.product_users.create') }}" class="btn btn-primary">Create ProductUser</a>
    <a href="{{ route('product.product_users.export') }}" class="btn btn-secondary">Export CSV</a>
    <form action="{{ route('product.product_users.import') }}" method="POST" enctype="multipart/form-data" style="display:inline-block;">
        @csrf
        <input type="file" name="csv_file" required>
        <button type="submit" class="btn btn-secondary">Import CSV</button>
    </form>
</div>
<form method="GET" action="{{ route('product.product_users.index') }}" class="toolbar" style="margin-top: 8px;">
    ...
    <a href="{{ route('product.product_users.index') }}" class="btn btn-secondary">Reset</a>
</form>
    ...
    <tbody>
        @forelse($productUsers as $item)
            <tr>
                ...
                <td>
                    <a href="{{ route('product.product_users.show', ['id' => $item->getKey()]) }}" class="btn btn-sm btn-primary">View</a>
                    <a href="{{ route('product.product_users.edit', ['id' => $item->getKey()]) }}" class="btn btn-sm btn-warning">Edit</a>
                    <form action="{{ route('product.product_users.destroy', ['id' => $item->getKey()]) }}" method="POST" style="display:inline;" data-ajax="delete">
                        @csrf
                        @method('DELETE')
